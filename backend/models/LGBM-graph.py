...
THEME_COLOR = '#00529b'  # Racing Blue
...
# ---------------------------------------------------------
# 4. THE LIGHTGBM DASHBOARD (BLUE & WHITE)
# ---------------------------------------------------------
...
ax1.set_title(f"LightGBM: Predicted vs Actual Delta\n(MAE: {mae:.4f}s)", fontsize=14, fontweight='bold')
...
ax3.set_xlabel("Total Gain (Reduction in Error)")
...
for (row, col), cell in tbl.get_celld().items():
    if row == 0:
        cell.set_facecolor('#dce6f2') # Light Blue header
        cell.set_text_props(weight='bold')

plt.suptitle("LIGHTGBM F1 TELEMETRY ANALYSIS: 2025 VALIDATION", fontsize=22, fontweight='bold', y=0.98, color=THEME_COLOR)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])

# Save
save_path = os.path.join(SAVE_DIR, "lgbm_blue_dashboard.png")
plt.savefig(save_path, dpi=300)

print("\n" + "="*50)
print("             LIGHTGBM FINAL REPORT (BLUE)")
print("="*50)
